...
def delete_key(client, workspace_id, name):
    try:
        client.delete_workspace_api_key(workspaceId=workspace_id, keyName=name)
    except Exception as e:
        print(f"Warning deleting key: {e}")
...
def handler(event, context):
    ...
    try:
        if event["RequestType"] in ["Create", "Update"]:
            ...
            call_grafana(endpoint, token, "POST", "/api/datasources", payload)
